import cv2
import mediapipe as mp
from dollarpy import Recognizer, Template, Point

# Initialize pose estimator
mp_drawing = mp.solutions.drawing_utils
mp_pose = mp.solutions.pose

pose = mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5)

# Set up camera capture
cap = cv2.VideoCapture(0)  # Use appropriate camera index for your system
framecnt = 0

# Define templates for your custom gestures

wave = Template('wave', [
    Point(117,469, 1),
    Point(380,453, 1),
    Point(113,462, 1),
    Point(378,454, 1),
    Point(80,403, 1),
    Point(382,455, 1),
    Point(132,402, 1),
    Point(378,452, 1),
    Point(126,317, 1),
    Point(381,457, 1),
    Point(119,292, 1),
    Point(381,458, 1),
    Point(100,270, 1),
    Point(384,460, 1),
    Point(89,259, 1),
    Point(383,461, 1),
    Point(78,247, 1),
    Point(384,459, 1),
    Point(72,224, 1),
    Point(382,459, 1),
    Point(72,218, 1),
    Point(382,457, 1),
    Point(71,217, 1),
    Point(380,456, 1),
    Point(72,218, 1),
    Point(381,456, 1),
    Point(73,217, 1),
    Point(381,456, 1),
    Point(73,221, 1),
    Point(382,456, 1),
    Point(74,221, 1),
    Point(382,455, 1),
    Point(74,220, 1),
    Point(380,455, 1),
    Point(77,219, 1),
    Point(380,456, 1),
    Point(81,220, 1),
    Point(381,458, 1),
    Point(84,219, 1),
    Point(382,459, 1),
    Point(85,219, 1),
    Point(384,458, 1),
    Point(86,219, 1),
    Point(390,456, 1),
    Point(86,218, 1),
    Point(390,457, 1),
    Point(86,219, 1),
    Point(390,454, 1),
    Point(85,219, 1),
    Point(390,453, 1),
    Point(81,219, 1),
    Point(389,453, 1),
    Point(77,220, 1),
    Point(388,454, 1),
    Point(75,220, 1),
    Point(387,454, 1),
    Point(74,220, 1),
    Point(387,454, 1),
    Point(74,220, 1),
    Point(387,454, 1),
    Point(74,221, 1),
    Point(387,454, 1),
    Point(75,221, 1),
    Point(385,454, 1),
    Point(77,220, 1),
    Point(383,458, 1),
    Point(82,220, 1),
    Point(381,458, 1),
    Point(87,219, 1),
    Point(381,464, 1),
    Point(90,219, 1),
    Point(381,463, 1),
    Point(92,219, 1),
    Point(381,463, 1),
    Point(92,219, 1),
    Point(381,463, 1),
    Point(93,219, 1),
    Point(382,463, 1),
    Point(93,219, 1),
    Point(383,462, 1),
    Point(92,219, 1),
    Point(383,461, 1),
    Point(92,219, 1),
    Point(383,460, 1),
    Point(92,220, 1),
    Point(383,460, 1),
    Point(92,219, 1),
    Point(383,460, 1),
    Point(92,220, 1),
    Point(383,460, 1),
    Point(92,220, 1),
    Point(382,460, 1),
    Point(92,220, 1),
    Point(382,459, 1),
    Point(91,223, 1),
    Point(382,458, 1),
    Point(86,226, 1),
    Point(382,458, 1),
    Point(82,227, 1),
    Point(383,457, 1),
    Point(80,229, 1),
    Point(383,457, 1),
    Point(80,229, 1),
    Point(381,462, 1),
    Point(80,229, 1),
    Point(381,462, 1),
    Point(81,229, 1),
    Point(381,461, 1),
    Point(86,226, 1),
    Point(381,459, 1),
    Point(91,224, 1),
    Point(381,459, 1),
    Point(94,223, 1),
    Point(381,459, 1),
    Point(95,222, 1),
    Point(381,459, 1),
    Point(95,222, 1),
    Point(381,459, 1),
    Point(95,222, 1),
    Point(381,459, 1),
    Point(95,222, 1),
    Point(381,459, 1),
    Point(95,222, 1),
    Point(381,459, 1),
    Point(95,223, 1),
    Point(381,460, 1),
    Point(95,223, 1),
    Point(381,459, 1),
    Point(94,224, 1),
    Point(382,459, 1),
    Point(88,230, 1),
    Point(382,458, 1),
    Point(87,229, 1),
    Point(382,458, 1),
    Point(85,229, 1),
    Point(381,457, 1),
    Point(84,229, 1),
    Point(382,457, 1),
    Point(84,229, 1),
    Point(382,456, 1),
    Point(84,228, 1),
    Point(382,456, 1),
    Point(84,227, 1),
    Point(383,454, 1),
    Point(84,227, 1),
    Point(383,454, 1),
    Point(85,227, 1),
    Point(383,454, 1),
    Point(87,225, 1),
    Point(383,454, 1),
    Point(90,219, 1),
    Point(382,454, 1),
    Point(93,218, 1),
    Point(381,457, 1),
    Point(94,220, 1),
    Point(381,458, 1),
    Point(96,223, 1),
    Point(380,460, 1),
    Point(97,224, 1),
    Point(380,461, 1),
    Point(97,224, 1),
    Point(380,463, 1),
    Point(98,224, 1),
    Point(381,462, 1),
    Point(97,225, 1),
    Point(384,462, 1),
    Point(97,226, 1),
    Point(384,462, 1),
    Point(97,226, 1),
    Point(385,463, 1),
    Point(97,225, 1),
    Point(384,462, 1),
    Point(97,225, 1),
    Point(384,462, 1),
    Point(97,225, 1),
    Point(387,461, 1),
    Point(98,224, 1),
    Point(386,461, 1),
    Point(98,223, 1),
    Point(385,461, 1),
    Point(100,222, 1),
    Point(385,460, 1),
    Point(100,222, 1),
    Point(385,460, 1),
    Point(100,222, 1),
    Point(385,459, 1),
    Point(102,221, 1),
    Point(385,459, 1),
    Point(104,219, 1),
    Point(385,460, 1),
    Point(105,219, 1),
    Point(385,460, 1),
    Point(105,219, 1),
    Point(385,461, 1),
    Point(104,219, 1),
    Point(385,461, 1),
    Point(102,224, 1),
    Point(385,462, 1),
    Point(99,236, 1),
    Point(386,464, 1),
    Point(93,256, 1),
    Point(385,465, 1),
    Point(111,456, 1),
    Point(382,465, 1),
    Point(118,476, 1),
    Point(379,465, 1),
    Point(116,453, 1),
    Point(380,463, 1),
    Point(114,460, 1),
    Point(381,464, 1),
    Point(113,463, 1),
    Point(381,464, 1),
    Point(113,470, 1),
    Point(381,464, 1),
    Point(112,463, 1),
    Point(381,464, 1),
    Point(112,460, 1),
    Point(381,464, 1),
    Point(111,461, 1),
    Point(382,463, 1),
])

thumbs_up = Template('thumbs_up', [
    Point(117,461, 1),
    Point(437,426, 1),
    Point(124,465, 1),
    Point(432,451, 1),
    Point(124,463, 1),
    Point(430,447, 1),
    Point(123,382, 1),
    Point(424,445, 1),
    Point(87,290, 1),
    Point(425,443, 1),
    Point(73,264, 1),
    Point(427,447, 1),
    Point(69,240, 1),
    Point(428,450, 1),
    Point(73,228, 1),
    Point(430,451, 1),
    Point(77,213, 1),
    Point(438,451, 1),
    Point(79,212, 1),
    Point(442,450, 1),
    Point(80,200, 1),
    Point(446,450, 1),
    Point(81,198, 1),
    Point(448,450, 1),
    Point(82,194, 1),
    Point(450,448, 1),
    Point(83,195, 1),
    Point(450,446, 1),
    Point(84,196, 1),
    Point(451,446, 1),
    Point(85,197, 1),
    Point(450,446, 1),
    Point(88,196, 1),
    Point(450,446, 1),
    Point(91,196, 1),
    Point(450,447, 1),
    Point(91,197, 1),
    Point(451,440, 1),
    Point(91,198, 1),
    Point(451,443, 1),
    Point(91,198, 1),
    Point(453,431, 1),
    Point(90,198, 1),
    Point(455,427, 1),
    Point(89,199, 1),
    Point(455,429, 1),
    Point(88,199, 1),
    Point(456,430, 1),
    Point(87,200, 1),
    Point(456,421, 1),
    Point(87,200, 1),
    Point(457,419, 1),
    Point(87,200, 1),
    Point(457,425, 1),
    Point(87,200, 1),
    Point(456,426, 1),
    Point(87,200, 1),
    Point(456,431, 1),
    Point(87,200, 1),
    Point(456,439, 1),
    Point(88,200, 1),
    Point(456,444, 1),
    Point(90,200, 1),
    Point(456,444, 1),
    Point(90,200, 1),
    Point(458,446, 1),
    Point(90,199, 1),
    Point(459,445, 1),
    Point(88,199, 1),
    Point(459,448, 1),
    Point(85,200, 1),
    Point(459,448, 1),
    Point(84,199, 1),
    Point(459,447, 1),
    Point(82,200, 1),
    Point(458,448, 1),
    Point(81,200, 1),
    Point(458,448, 1),
    Point(81,200, 1),
    Point(457,448, 1),
    Point(81,200, 1),
    Point(460,433, 1),
    Point(81,200, 1),
    Point(460,437, 1),
    Point(81,200, 1),
    Point(460,437, 1),
    Point(81,200, 1),
    Point(461,437, 1),
    Point(81,199, 1),
    Point(460,439, 1),
    Point(81,197, 1),
    Point(460,432, 1),
    Point(81,197, 1),
    Point(460,433, 1),
    Point(81,197, 1),
    Point(458,439, 1),
    Point(81,196, 1),
    Point(458,439, 1),
    Point(82,195, 1),
    Point(458,439, 1),
    Point(82,195, 1),
    Point(458,440, 1),
    Point(83,196, 1),
    Point(458,446, 1),
    Point(84,195, 1),
    Point(458,442, 1),
    Point(86,195, 1),
    Point(458,440, 1),
    Point(87,196, 1),
    Point(459,439, 1),
    Point(88,196, 1),
    Point(459,445, 1),
    Point(88,196, 1),
    Point(459,445, 1),
    Point(89,196, 1),
    Point(459,446, 1),
    Point(89,197, 1),
    Point(459,446, 1),
    Point(88,198, 1),
    Point(459,446, 1),
    Point(89,197, 1),
    Point(459,446, 1),
    Point(88,197, 1),
    Point(459,446, 1),
    Point(88,198, 1),
    Point(460,448, 1),
    Point(88,198, 1),
    Point(461,448, 1),
    Point(88,198, 1),
    Point(463,446, 1),
    Point(88,198, 1),
    Point(464,445, 1),
    Point(87,198, 1),
    Point(465,440, 1),
    Point(87,198, 1),
    Point(466,440, 1),
    Point(87,198, 1),
    Point(466,437, 1),
    Point(87,198, 1),
    Point(466,436, 1),
    Point(87,199, 1),
    Point(466,434, 1),
    Point(87,200, 1),
    Point(466,434, 1),
    Point(87,200, 1),
    Point(466,433, 1),
    Point(87,201, 1),
    Point(467,432, 1),
    Point(87,201, 1),
    Point(466,436, 1),
    Point(87,201, 1),
    Point(465,440, 1),
    Point(87,201, 1),
    Point(465,441, 1),
    Point(87,201, 1),
    Point(465,441, 1),
    Point(87,201, 1),
    Point(467,438, 1),
    Point(88,202, 1),
    Point(466,444, 1),
    Point(87,204, 1),
    Point(463,448, 1),
    Point(87,204, 1),
    Point(462,449, 1),
    Point(85,204, 1),
    Point(460,452, 1),
    Point(84,205, 1),
    Point(459,451, 1),
    Point(84,205, 1),
    Point(457,451, 1),
    Point(83,205, 1),
    Point(457,451, 1),
    Point(83,205, 1),
    Point(457,449, 1),
    Point(83,205, 1),
    Point(457,448, 1),
    Point(83,205, 1),
    Point(457,448, 1),
    Point(83,205, 1),
    Point(457,448, 1),
    Point(82,206, 1),
    Point(459,438, 1),
    Point(81,206, 1),
    Point(460,438, 1),
    Point(81,206, 1),
    Point(463,427, 1),
    Point(81,205, 1),
    Point(463,424, 1),
    Point(83,204, 1),
    Point(464,424, 1),
    Point(85,203, 1),
    Point(463,433, 1),
    Point(87,202, 1),
    Point(463,430, 1),
    Point(87,202, 1),
    Point(463,430, 1),
    Point(88,201, 1),
    Point(463,424, 1),
    Point(88,201, 1),
    Point(459,436, 1),
    Point(88,200, 1),
    Point(460,428, 1),
    Point(89,198, 1),
    Point(460,427, 1),
    Point(89,196, 1),
    Point(466,407, 1),
    Point(89,196, 1),
    Point(466,411, 1),
    Point(89,195, 1),
    Point(466,413, 1),
    Point(89,195, 1),
    Point(465,415, 1),
    Point(89,196, 1),
    Point(465,415, 1),
    Point(88,196, 1),
    Point(465,418, 1),
    Point(88,196, 1),
    Point(467,409, 1),
    Point(87,197, 1),
    Point(466,413, 1),
    Point(86,197, 1),
    Point(466,417, 1),
    Point(85,197, 1),
    Point(466,419, 1),
    Point(82,197, 1),
    Point(466,419, 1),
    Point(78,197, 1),
    Point(467,419, 1),
    Point(77,197, 1),
    Point(467,417, 1),
    Point(75,196, 1),
    Point(467,417, 1),
    Point(74,196, 1),
    Point(467,416, 1),
    Point(74,196, 1),
    Point(467,415, 1),
    Point(74,193, 1),
    Point(465,413, 1),
    Point(74,193, 1),
    Point(464,418, 1),
    Point(74,194, 1),
    Point(463,421, 1),
    Point(74,194, 1),
    Point(460,435, 1),
    Point(74,195, 1),
    Point(459,435, 1),
    Point(76,196, 1),
    Point(459,436, 1),
    Point(77,195, 1),
    Point(458,438, 1),
    Point(86,197, 1),
    Point(461,427, 1),
    Point(89,216, 1),
    Point(463,407, 1),
    Point(98,270, 1),
    Point(457,410, 1),
    Point(118,287, 1),
    Point(442,408, 1),
    Point(114,448, 1),
    Point(443,422, 1),
    Point(109,465, 1),
    Point(444,420, 1),
    Point(107,471, 1),
    Point(445,427, 1),
    Point(106,471, 1),
    Point(448,432, 1),
    Point(106,472, 1),
    Point(452,431, 1),
    Point(107,474, 1),
    Point(454,429, 1),
    Point(109,474, 1),
    Point(455,429, 1),
    Point(115,475, 1),
    Point(455,429, 1),
    Point(115,472, 1),
    Point(455,436, 1),
    Point(115,471, 1),
    Point(455,430, 1),
])

# Recognizer setup
recognizer = Recognizer([wave, thumbs_up])

Allpoints = []

while cap.isOpened():
    ret, frame = cap.read()
    if not ret:
        print("Can't receive frame (stream end?). Exiting ...")
        break

    frame = cv2.resize(frame, (480, 320))
    framecnt += 1

    try:
        # Convert the frame to RGB
        RGB = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        # Process the frame for pose landmarks
        results = pose.process(RGB)

        if results.pose_landmarks:
            image_height, image_width, _ = frame.shape

            # Capture the wrist landmarks for gesture recognition
            right_wrist_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST].x * image_width)
            right_wrist_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.RIGHT_WRIST].y * image_height)
            left_wrist_x = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_WRIST].x * image_width)
            left_wrist_y = int(results.pose_landmarks.landmark[mp_pose.PoseLandmark.LEFT_WRIST].y * image_height)

            # Append wrist landmarks to the Allpoints list
            Allpoints.append(Point(right_wrist_x, right_wrist_y, 1))
            Allpoints.append(Point(left_wrist_x, left_wrist_y, 1))

            # Recognize gesture every 20 frames
            if framecnt % 20 == 0:
                framecnt = 0
                result = recognizer.recognize(Allpoints)
                print(f"Gesture recognized: {result}")
                Allpoints.clear()

            # Draw pose landmarks
            mp_drawing.draw_landmarks(frame, results.pose_landmarks, mp_pose.POSE_CONNECTIONS)

        # Show the result
        cv2.imshow('Output', frame)

    except Exception as e:
        print(f"Error: {e}")

    # Break on 'q' key press
    if cv2.waitKey(1) == ord('q'):
        break

cap.release()
cv2.destroyAllWindows()
